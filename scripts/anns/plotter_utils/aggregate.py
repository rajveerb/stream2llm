...
def generate_metric_time_stats(streaming_times, non_streaming_times,
    ...
    """
    Generate statistics about the query metrics for ANNS experiments.
...
def extract_delay_multiplier(run_dir) -> float:
    """Extract the delay multiplier from the config file in the run directory."""
    config_file = os.path.join(run_dir,
                               f"config_{os.path.basename(run_dir)}.yaml")
    ...
    try:
        with open(config_file, 'r') as f:
            config = yaml.safe_load(f)
        return config.get('replay', {}).get('delay_multiplier', 1.0)
    except (FileNotFoundError, yaml.YAMLError, KeyError):
        return 1.0


def extract_scheduler(run_dir) -> str:
    """Extract the scheduler name from the config file in the run directory."""
    config_file = os.path.join(run_dir,
                               f"config_{os.path.basename(run_dir)}.yaml")
    ...
    try:
        ...
        return scheduler if scheduler else 'unknown'
    except (FileNotFoundError, yaml.YAMLError, KeyError):
        return 'unknown'
...
def process_trace_time_data(run_dirs, streaming_times, non_streaming_times):
    """
    Process trace time data for plotting in ANNS experiments.
...
def plot_speedup_vs_replay_rate(replay_rates, speedups, run_labels,
                                output_dir):
    """Plot the trace completion time ratio (speedup) against replay rate for ANNS."""
    ...
    plt.title('ANNS Trace Completion Time Speedup vs Replay Rate')
...
def plot_raw_trace_times(replay_rates, streaming_values, non_streaming_values,
    ...
    """Plot the raw streaming and non-streaming trace end times for each replay rate in ANNS."""
    ...
    plt.title('ANNS Raw Trace Completion Times vs Replay Rate')
    plt.grid(True, alpha=0.3)
    plt.legend()
...
def plot_preemptions_vs_replay_rate(preemption_data, output_dir):
    """Plot preemption counts vs replay rate for ANNS experiments.
    ...
    plt.title('ANNS Preemption Counts vs Replay Rate')

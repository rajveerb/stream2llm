...
def extract_query_e2e_latencies(
    ...
    """Extract ANNS query e2e completion times from the metrics_file file."""
    df = pd.read_csv(metrics_file, low_memory=False)

    # Filter for query_e2e_latency events (from replay_anns_queries.py)
...
def plot_query_e2e_times_histogram(streaming_e2e_times: np.ndarray,
    ...
    """Plot histograms of ANNS query completion times."""
    ...
    title = 'Distribution of ANNS Query E2E Completion Times'
    if title_suffix:
...
def plot_query_e2e_times_cdf(streaming_e2e_times,
                             ...
                             output_path,
                             title_suffix=""):
    """Plot CDF of ANNS query completion times."""
    # Create a figure with two subplots - one for the plot and one for metrics text
    _ = plt.figure(figsize=(15, 8))
    ...
    ax1.set_xlabel('Query E2E Completion Time (seconds)')
    ax1.set_ylabel('CDF')
    title = 'CDF of ANNS Query E2E Completion Times'
    if title_suffix:
        title += f' {title_suffix}'
    ax1.set_title(title)
    ax1.legend()

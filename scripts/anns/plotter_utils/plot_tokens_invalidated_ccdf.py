#!/usr/bin/env python3
"""plot_tokens_invalidated_ccdf.py
Generate CCDF (Complementary CDF) plots showing token invalidation tail behavior.

Creates plots showing P(tokens_invalidated > x) to visualize tail distributions
and how efficiently each scheduler manages cache under update mode.

Usage:
    python driver/anns/plotter_utils/plot_tokens_invalidated_ccdf.py \
...
def plot_tokens_invalidated_ccdf_combined(data, output_dir: Path, title_suffix: str = ""):
    """Create combined CCDF plots for token invalidation at different QPS levels."""
    ...
    for idx, qps_val in enumerate(qps_to_plot):
        ...
        for sched in sorted(data.keys()):
            ...
            # CCDF: 1 - CDF, so we start from 1 and go down
            y = (len(tokens_sorted) - np.arange(len(tokens_sorted))) / len(tokens_sorted)
    ...
    # Overall title
    fig.suptitle(f"Token Invalidation CCDF Across Schedulers{title_suffix}",
                 fontsize=16, fontweight='bold', y=0.98)

    fig.tight_layout(rect=[0, 0, 1, 0.96])
    output_dir.mkdir(parents=True, exist_ok=True)

    filename = "tokens_invalidated_ccdf_combined.png"
...
def plot_tokens_invalidated_ccdf_log_scale(data, output_dir: Path, title_suffix: str = ""):
    """Create CCDF plots with log scale on y-axis for better tail visualization."""
    ...
    # Overall title
    fig.suptitle(f"Token Invalidation CCDF Across Schedulers (Log Scale){title_suffix}",
                 fontsize=16, fontweight='bold', y=0.98)

    fig.tight_layout(rect=[0, 0, 1, 0.96])
    output_dir.mkdir(parents=True, exist_ok=True)

    filename = "tokens_invalidated_ccdf_combined_log.png"
    fig.savefig(output_dir / filename, dpi=300, bbox_inches='tight')
    plt.close(fig)
...
def main(argv: Sequence[str] | None = None):
    parser = argparse.ArgumentParser(
        description="Generate token invalidation CCDF plots")
    ...
    # Generate both linear and log scale CCDF plots
    plot_tokens_invalidated_ccdf_combined(data, args.output_dir, args.title_suffix)
    plot_tokens_invalidated_ccdf_log_scale(data, args.output_dir, args.title_suffix)

#!/usr/bin/env python3
"""plot_latency_vs_qps.py
Generate latency vs QPS plots for P50 and P99 percentiles.
...
def plot_latency_vs_qps(data, output_dir: Path, title_suffix: str = ""):
    ...
    for sched in sorted(data.keys()):
        ...
        for qps in all_qps:
            ...
            # Skip if no data
            if len(streaming) == 0 or len(non_streaming) == 0:
                continue
    ...
    ax_p50.grid(True, alpha=0.3, which='both')
    ...
    ax_p99.grid(True, alpha=0.3, which='both')
...
def main(argv: Sequence[str] | None = None):
    parser = argparse.ArgumentParser(
        description="Generate latency vs QPS plots for P50 and P99")

#!/usr/bin/env python3
"""plot_ttft_ccdf_combined_no_pbas.py
Generate combined CCDF plots with all schedulers except oeda_pbas.
...
def _combined_plot_per_qps(data, qps: float, out_dir: Path):
    """Create a combined plot with all schedulers (except oeda_pbas) for a specific QPS value."""
    ...
    # Set up axes
    ax.set_xlabel("TTFT (s)", fontsize=26, fontweight='bold')
...
def main(argv: Sequence[str] | None = None):
    parser = argparse.ArgumentParser(
        description=
        "Generate combined CCDF plots showing all schedulers (except oeda_pbas) per QPS value")
    ...
    # Generate one plot per QPS with all schedulers (except oeda_pbas)
    for qps in all_qps:

...
def _cdf_adaptive_color(ax, baseline_arr: TTFTArray, comp_arr: TTFTArray,
                        label: str, **style):
    """Plot comparison CCDF with adaptive coloring: green where better, red where worse.

    For CCDF (1-CDF), lower values are better (fewer requests still waiting).
    Green: comparison CCDF < baseline CCDF (better - steeper decay)
    Red: comparison CCDF > baseline CCDF (worse - heavier tail)
    """
    ...
    comp_ccdf = 1 - comp_cdf  # Complementary CDF
    ...
    baseline_ccdf_interp = 1 - baseline_cdf_interp  # Complementary CDF

    # Determine color for each point: green if comp_ccdf < baseline_ccdf (better), red otherwise
    colors = [
        'green' if comp_y < baseline_y else 'red'
        for comp_y, baseline_y in zip(comp_ccdf, baseline_ccdf_interp)
    ]

    # Plot line segments with appropriate colors
    for i in range(len(comp_srt) - 1):
        ax.plot([comp_srt[i], comp_srt[i + 1]],
                [comp_ccdf[i], comp_ccdf[i + 1]],
                color=colors[i],
...
def _grid(data, qps_val: float, out_dir: Path):
    ...
    for idx, (sched, mode, title) in enumerate(comps):
        ...
        ax.set_ylim(1e-4, 1)
        ax.invert_yaxis(
        )  # Invert y-axis: 100% at top, smaller values at bottom

        # Set custom y-axis tick labels as percentages
    ...
    fig.suptitle(f"TTFT CCDF Grid (Tail Distribution) — QPS {qps_val:.3f}",
                 fontsize=16)

...
def plot_trace_completion_vs_qps(metrics_files: List[str], output_dir: str):
    """Create line plots showing trace completion time vs QPS for different schedulers."""
    ...
    # Create combined plot with all schedulers
    ...
    # Plot streaming trace completion times
    for scheduler_name in sorted(data.keys()):
        ...
        if x_positions and streaming_times:
    ...
    ax1.set_xlabel('QPS (Queries Per Second)', fontsize=12)
    ax1.set_ylabel('Trace Completion Time (seconds)', fontsize=12)
    ax1.set_title('Streaming Trace Completion Time vs QPS',
                  fontsize=14,
                  fontweight='bold')
    ...
    ax1.set_xticks(range(len(sorted_rates)))
    ax1.set_xticklabels([f'{rate:.3f}' for rate in sorted_rates], rotation=45)

    # Plot non-streaming trace completion times
    for scheduler_name in sorted(data.keys()):
        x_positions = []
        non_streaming_times = []
        ...
        if x_positions and non_streaming_times:
            color = scheduler_colors.get(scheduler_name, '#000000')
    ...
    ax2.set_xlabel('QPS (Queries Per Second)', fontsize=12)
    ax2.set_ylabel('Trace Completion Time (seconds)', fontsize=12)
    ax2.set_title('Non-Streaming Trace Completion Time vs QPS',
                  fontsize=14,
                  fontweight='bold')
    ...
    # Set equally spaced x-axis ticks with QPS labels
    ax2.set_xticks(range(len(sorted_rates)))
    ax2.set_xticklabels([f'{rate:.3f}' for rate in sorted_rates], rotation=45)

    # Overall title
    fig.suptitle('Trace Completion Time vs QPS by Scheduler',
                 fontsize=16,
                 fontweight='bold')

    # Save plot
    plt.tight_layout()
    filename = 'trace_completion_vs_qps_comparison.png'
    filepath = os.path.join(output_dir, filename)
    plt.savefig(filepath, bbox_inches='tight', dpi=300)
    plt.close()

    print(f"Trace completion vs QPS plot saved: {filepath}")
...
def main():
    ...
    if args.speedup_only:
        ...
        plot_trace_completion_vs_qps(metrics_files, args.output_dir)
        plot_trace_completion_combined(metrics_files, args.output_dir)
        plot_trace_completion_speedup(metrics_files, args.output_dir)

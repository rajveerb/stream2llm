...
class AsyncMPClient(MPClient):
    ...
    def __init__(self, vllm_config: VllmConfig, executor_class: type[Executor],
        ...
        self.client_input_stream_queue: asyncio.Queue[
            asyncio.Queue[EngineCoreRequest]] = asyncio.Queue()
        self.req_id_to_output_queue: Dict[
            str, asyncio.Queue[EngineCoreOutput]] = {}
        # create a task to loop over the client_input_stream_queue
        asyncio.create_task(self.loop_over_requests_queue_async())

    async def _start_output_queue_task(self):
        # Perform IO in separate task to parallelize as much as possible.
        # Avoid task having direct reference back to the client.
        self.outputs_queue = asyncio.Queue()
        output_socket = make_zmq_socket(self.ctx, self.output_path,
                                        zmq.constants.PULL)
        ...
        async def process_outputs_socket():
            while True:
                ...
                outputs: EngineCoreOutputs = self.decoder.decode(frame.buffer)
                if outputs.utility_output:
                    _process_utility_output(outputs.utility_output,
                    ...
                    for output in outputs.outputs:
                        self.req_id_to_output_queue[
                            output.request_id].put_nowait(output)
    ...
    ) -> Union[EngineCoreOutput, EngineCoreOutputs]:
        if self.outputs_queue is None:
            await self._start_output_queue_task()
            assert self.outputs_queue is not None
        if request_id is None:
            return await self.outputs_queue.get()
        return await self.req_id_to_output_queue[request_id].get()

    async def loop_over_requests_queue_async(self) -> None:

        while (request_input_stream_queue := await
               self.client_input_stream_queue.get()):
            asyncio.create_task(
                self.loop_over_input_stream_async(request_input_stream_queue))

    async def loop_over_input_stream_async(
            self, request_input_stream_queue: asyncio.Queue[EngineCoreRequest]
    ) -> None:
        while True:
            request = await request_input_stream_queue.get()
            if request is None:
                # Signal end of this stream
                request_input_stream_queue.task_done()
                break
            request.prompt = None
            await self._send_input(EngineCoreRequestType.STREAM_ADD, request)
            request_input_stream_queue.task_done()
    ...
    async def add_request_async(self, request: EngineCoreRequest) -> None:
        ...
        request_output_stream_queue: asyncio.Queue[
            EngineCoreOutput] = asyncio.Queue()
        self.req_id_to_output_queue[
            request.request_id] = request_output_stream_queue

    async def create_stream_request_async(
        self, request: EngineCoreRequest
    ) -> Tuple[asyncio.Queue[EngineCoreRequest],
               asyncio.Queue[EngineCoreOutput]]:
        # NOTE: text prompt is not needed in the core engine as it has been
        # tokenized.
        request.prompt = None
        request_input_stream_queue: asyncio.Queue[
            EngineCoreRequest] = asyncio.Queue()
        request_output_stream_queue: asyncio.Queue[
            EngineCoreOutput] = asyncio.Queue()
        self.req_id_to_output_queue[
            request.request_id] = request_output_stream_queue
        self.client_input_stream_queue.put_nowait(request_input_stream_queue)
        await self._send_input(EngineCoreRequestType.STREAM_ADD, request)
        return request_input_stream_queue, request_output_stream_queue
    ...
    def shutdown(self) -> None:
        # gracefully shutdown the client_input_stream_queue and its tasks
        while not self.client_input_stream_queue.empty():
            request_queue = self.client_input_stream_queue.get_nowait()

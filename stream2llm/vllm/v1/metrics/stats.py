    ...
    def update_from_events(self, req_id: str, events: list["EngineCoreEvent"],
        ...
        for event in events:
            if event.type == EngineCoreEventType.QUEUED:
                req_stats.queued_ts = event.start_timestamp
                if lora_stats is not None:
                    lora_stats.waiting_requests.add(req_id)
            elif event.type == EngineCoreEventType.SCHEDULED:
                if req_stats.scheduled_ts == 0.0:  # ignore preemptions
                    req_stats.scheduled_ts = event.start_timestamp
                LoRARequestStates.scheduled_request(lora_stats, req_id)
            #TODO: rbachkanwiwala3 add PREEMPTED_SWAP
            elif event.type == EngineCoreEventType.PREEMPTED_RECOMPUTE:

...
class GroupCoordinator:
    ...
    def destroy(self):
        if self.device_group is not None:
            _safe_destroy_process_group("device", lambda: torch.distributed.destroy_process_group(self.device_group))
            self.device_group = None
        if self.cpu_group is not None:
            _safe_destroy_process_group("cpu", lambda: torch.distributed.destroy_process_group(self.cpu_group))
            self.cpu_group = None
        if self.device_communicator is not None:
...
def _safe_destroy_process_group(group_name: str, destroy_func: callable):
    """Safely destroy a process group with proper error handling and memory cleanup."""
    # Clear CUDA cache before destroying process groups to prevent OOM
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
        torch.cuda.synchronize()
    
    # Check if we should skip process group destruction (for debugging)
    if os.environ.get("VLLM_SKIP_PG_DESTROY", "0") == "1":
        logger.info(f"Skipping {group_name} process group destruction due to VLLM_SKIP_PG_DESTROY=1")
        return
        
    destroy_func()
    logger.debug(f"Successfully destroyed {group_name} process group")

def destroy_model_parallel():
    """Set the groups to none and destroy them."""
    global _TP
    if _TP:
        _safe_destroy_process_group("tensor parallel", _TP.destroy)
        _TP = None

    global _PP
    if _PP:
        _safe_destroy_process_group("pipeline parallel", _PP.destroy)
        _PP = None

    global _DP
    if _DP:
        _safe_destroy_process_group("data parallel", _DP.destroy)
        _DP = None
...
def destroy_distributed_environment():
    global _WORLD
    if _WORLD:
        _safe_destroy_process_group("world", _WORLD.destroy)
    _WORLD = None

...
def get_vllm_version() -> str:
    version = os.getenv(
        "VLLM_VERSION",
        "0.8.1")  # Static version instead of using setuptools-scm
    ...
    # Create the _version.py file with the calculated version
    create_version_file_with_version(version)

    return version


def create_version_file_with_version(version: str) -> None:
    """Create the _version.py file with the provided version information."""
    version_file_path = ROOT_DIR / "vllm" / "_version.py"

    # Parse version tuple, handling complex version strings
    base_version = version.split("+")[0].split(
        ".") if "+" in version else version.split(".")
    version_tuple = []
    ...
    # Ensure we have at least 3 parts for the tuple
    while len(version_tuple) < 3:
        version_tuple.append(0)

    version_content = f'''# SPDX-License-Identifier: Apache-2.0
# Auto-generated version file - DO NOT EDIT MANUALLY
# This file is created by setup.py during build/install

__version__ = "{version}"
__version_tuple__ = {tuple(version_tuple[:3])}
'''
    ...
    try:
        ...
        print(
            f"Warning: Could not create version file {version_file_path}: {e}")
